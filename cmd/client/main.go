...
import (
	...
	"cyber-range-config/internal/client"
...
func main() {
	...
	// Set up logging
	if err := client.EnsureMarkerDir(); err != nil {
		fmt.Fprintf(os.Stderr, "Failed to create log directory: %v\n", err)
		os.Exit(1)
	}

	logFile, err := os.OpenFile(client.GetLogPath(), os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
	...
	log.Println("=== Cyber Range Configuration Client Starting ===")
	...
	var mac string
	if *interfaceName != "" {
		mac, err = client.GetMACByName(*interfaceName)
	} else {
		mac, err = client.GetPrimaryMAC()
	}
	if err != nil {
	...
	if err := client.SetHostname(cfg.Hostname); err != nil {
	...
	if err := client.ConfigureNetwork(cfg.Network); err != nil {
		log.Fatalf("Failed to configure network: %v", err)
	}
	log.Println("Network configured successfully")

	// Create marker file
	if err := client.CreateMarker(cfg.Hostname); err != nil {
